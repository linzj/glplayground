...
static void
_PezFatalError(const char* pStr, va_list a)
{
  char msg[1024] = { 0 };
...
static void
PezCheckCondition(int condition, ...)
{
  va_list a;
  const char* pStr;
...
PezRender(void)
{
  ...
  float packed[9] = { nm.col0.x, nm.col1.x, nm.col2.x, nm.col0.y, nm.col1.y,
                      nm.col2.y, nm.col0.z, nm.col1.z, nm.col2.z };
...
PezInitialize(void)
{
  ...
  Uniforms.AmbientMaterial =
    glGetUniformLocation(ProgramHandle, "AmbientMaterial");
  Uniforms.DiffuseMaterial =
    glGetUniformLocation(ProgramHandle, "DiffuseMaterial");
  Uniforms.TessLevelInner =
    glGetUniformLocation(ProgramHandle, "TessLevelInner");
  Uniforms.TessLevelOuter =
    glGetUniformLocation(ProgramHandle, "TessLevelOuter");

  // Set up the projection matrix:
  const float HalfWidth = 0.6f;
  const float HalfHeight = HalfWidth * PEZ_VIEWPORT_HEIGHT / PEZ_VIEWPORT_WIDTH;
  ProjectionMatrix =
    M4MakeFrustum(-HalfWidth, +HalfWidth, -HalfHeight, +HalfHeight, 5, 150);
...
CreateIcosahedron()
{
  const int Faces[] = { 2,  1, 0, 3,  2, 0, 4,  3, 0, 5,  4,  0,  1,  5,  0,
  ...
  const float Verts[] = {
    0.000f,  0.000f,  1.000f,  0.894f,  0.000f,  0.447f,  0.276f, 0.851f,
    0.447f,  -0.724f, 0.526f,  0.447f,  -0.724f, -0.526f, 0.447f, 0.276f,
    -0.851f, 0.447f,  0.724f,  0.526f,  -0.447f, -0.276f, 0.851f, -0.447f,
    -0.894f, 0.000f,  -0.447f, -0.276f, -0.851f, -0.447f, 0.724f, -0.526f,
    -0.447f, 0.000f,  0.000f,  -1.000f
  };
...
LoadEffect()
{
  ...
  const char* msg =
    "Can't find %s shader.  Does '../BicubicPath.glsl' exist?\n";
...
static void
PezUpdate(unsigned int elapsedMicroseconds)
{
