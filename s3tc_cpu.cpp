...
static int etc1_decode_image(const etc1_byte* pIn, etc1_byte* pOut,
...
static void initTextureDatas(void)
{
    ...
    header = (unsigned char*)malloc(inputstat.st_size);
    if (!header) {
...
const char *fragmentShaderSource[] = {
   ...
   "void main(void)\n",
   "{\n",
   "   fg_FragColor = texture(myt, vec2((tex_coord.x + 1.0) / 2.0, 1.0 - (tex_coord.y + 1.0) / 2.0));\n",
   "}\n"
};
...
void init(void) 
{
   ...
   checkError ("before initTextureDatas");
   initTextureDatas();
   initBuffer ();
   initShader ();
   initRendering ();
...
static void triangle_normal(void)
{
   ...
   data = (unsigned char*)malloc(encoded_width * encoded_height * 3);
   etc1_decode_image((const etc1_byte*)textureData, data, encoded_width, encoded_height, 3, encoded_width * 3);
   imageSize = squish::GetStorageRequirements(encoded_width, encoded_height, squish::kDxt1);
   pOutCompress = new uint8_t[imageSize];
   squish::CompressImage24(static_cast<const squish::u8*>(data), encoded_width, encoded_height, pOutCompress);
   glCompressedTexImage2D(GL_TEXTURE_2D, 0, GL_COMPRESSED_RGB_S3TC_DXT1_EXT, encoded_width, encoded_height, 0, encoded_width * encoded_height / 2, pOutCompress);
   if (data) {
       free(data);
   }
   delete pOutCompress;
